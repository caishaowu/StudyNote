...
--50、查询第n高的成绩
select distinct degree from score order by degree desc limit n-1,1;

--51、行转列
select year,
max(if(`month` = 1,count,0)) as m1, 
max(if(`month` = 2,count,0)) as m2, 
max(if(`month` = 3,count,0)) as m3, 
max(if(`month` = 4,count,0)) as m4 
from birth 
group by year;
或者
SELECT year,
SUM(CASE `month` WHEN 1 THEN count ELSE 0 END) as '语文',
SUM(CASE `month` WHEN 2 THEN count ELSE 0 END) as '数学',
SUM(CASE `month` WHEN 3 THEN count ELSE 0 END) as '英语',
SUM(CASE `month` WHEN 4 THEN count ELSE 0 END) as '政治' 
FROM birth 
GROUP BY year;
