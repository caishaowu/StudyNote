...
**logback-spring.xml**：**日志框架**就不直接加载日志的配置项，由**SpringBoot解析日志**配置，可以使用SpringBoot的高级Profile功能
...
![1550303515085](C:\Users\csw\AppData\Roaming\Typora\typora-user-images\1550303515085.png)
...
Simple expressions:（表达式语法）
    Variable Expressions: ${...}：获取变量值；OGNL；
    		1）、获取对象的属性、调用方法 
    		${person.name}、${person.createCompleteName()}
    		2）、使用内置的基本对象：
...
#ids : methods for dealing with id attributes that might be repeated (for example, as a result of an iteration).
   ...
   <div>
    ...
    Link URL Expressions: @{...}：定义URL；
    		/projectName/order/process?execId=3
...
SpringBoot2.x需要加上Spring.datasource.initialization-mode=always;
spring.datasource.type: 配置数据源
```
...
自定义启动器命名规则：mybatis-spring-boot-starter；自定义启动器名-spring-boot-starter
